...
protocol BusArrivalRemoteDataSource {
    ...
    func getArrivalInfoList(cityCode: String, nodeId: String) async throws -> [BusStationArrivalInfoDTO]
}
...
final class DefaultBusArrivalRemoteDataSource: BusArrivalRemoteDataSource {
    ...
    func getArrivalInfoList(cityCode: String, nodeId: String) async throws -> [BusStationArrivalInfoDTO] {
        try await networkService.request(.getArrivalInfoList(cityCode: cityCode, nodeId: nodeId), type: BusStationArrivalInfoListResponseDTO.self).response.body.items
    }
}
