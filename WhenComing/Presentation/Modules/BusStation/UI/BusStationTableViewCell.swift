...
//            make.height.equalTo(60)
    ...
    func configure(busNumber: String, busType: String, busTime: Int?, busStationAgo: Int?) {
        ...
        if let busTime = busTime, let busStationAgo = busStationAgo {
            ...
            let timeText = min > 9 ? "\(min)분 (\(busStationAgo)역 전)" : (min == 0 ? "곧 도착" : "\(min)분 \(seconds)초 (\(busStationAgo)역 전)")
    ...
    func setFavorite(_ isFavorite: Bool) {
        starImage.image = isFavorite ? UIImage(systemName: "star.fill") : UIImage(systemName: "star")
        starImage.tintColor = isFavorite ? .systemYellow : .systemGray4
    }
}
